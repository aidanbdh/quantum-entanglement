...
module.exports = function(subscriptions, text, connected, link) {
  ...
  const onConnect = function(notification) {
    ...
    socket.on('disconnect', (user = null) => {
      if(user === null) return
      cursors.forEach((value, id) => {
        if(value.color[0] == user.color) {
          notification(`${value.name} has disconnected`)
          value.cursor.destroy()
          cursors.splice(id, id + 1)
          return
        }
      })
    })
